...
watch(() => store.dialog.open, (val) => {
  if (val) {
    form.value = store.dialog.mode === 'edit'
      ? { name: store.dialog.item.name, address: store.dialog.item.address, isActive: store.dialog.item.isActive }
...
const submit = () => {
  if (store.dialog.mode === 'edit') {
    store.updateParty(store.dialog.item._id, form.value)
  } else {
    store.createParty({ name: form.value.name, address: form.value.address })
  }
}
</script>
      ...
      <div class="relative w-full bg-card-background border-t border-primary-border rounded-t-2xl p-5 z-10">
        ...
        <div class="flex items-center justify-between mb-5">
          <h2 class="title-text text-primary-text">
            {{ store.dialog.mode === 'edit' ? 'Edit Party' : 'Create Party' }}
          </h2>
        ...
        <div class="flex flex-col gap-4">
          <div>
            ...
            <input v-model="form.name" type="text" placeholder="Party name" class="input-field w-full" />
          </div>
          <div>
            <label class="label-text text-secondary-text block mb-1">Address</label>
          ...
          <button
            ...
            class="w-full py-3 rounded-lg bg-primary-accent text-primary-text mid-text disabled:opacity-50"
          >
